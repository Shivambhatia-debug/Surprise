...
export default function GiftBoxActivity({ data, onNext, isLast }: { data: DoorData; onNext: () => void; isLast: boolean }) {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      {!opened ? (
        <motion.div onClick={handleOpen} style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center' }}
          ...
          <div style={{
            width: 120, height: 120, background: 'linear-gradient(135deg, #ff1493, #ff69b4)', borderRadius: 16,
          ...
          <p style={{ color: '#fff', marginTop: 20, fontWeight: 700, fontSize: '1.1rem', letterSpacing: 2, textTransform: 'uppercase',
            textShadow: '0 0 10px rgba(255,105,180,0.8)', animation: 'pulseText 2s infinite' }}>TAP TO OPEN GIFT {data.id}</p>
        </motion.div>
      ) : (

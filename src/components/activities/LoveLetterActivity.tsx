...
export default function LoveLetterActivity({ data, onNext, isLast }: { data: DoorData; onNext: () => void; isLast: boolean }) {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      {!opened ? (
        <motion.div onClick={() => setOpened(true)} style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center' }}
          ...
          <div style={{
            width: 180, height: 130, background: 'linear-gradient(135deg, #ff69b4, #ff1493)',
            ...
            <div style={{ position: 'absolute', top: 0, left: 0, width: 0, height: 0,
              ...
              borderTop: '65px solid rgba(255,255,255,0.25)', pointerEvents: 'none' }} />
            <div style={{ width: 45, height: 45, borderRadius: '50%', background: '#ffd700',
              display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2,
              boxShadow: '0 3px 15px rgba(255,215,0,0.6)' }}>
              <Mail size={20} color="#fff" />
            </div>
          </div>
          <p style={{ color: '#fff', marginTop: 20, fontWeight: 700, fontSize: '1.1rem', letterSpacing: 2,
